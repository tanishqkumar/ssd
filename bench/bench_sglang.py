...
# Handle nested asyncio loops if running in Jupyter or similar environments
try:
    import nest_asyncio
    nest_asyncio.apply()
except ImportError:
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark SGLang with Qwen/Llama models and optional speculative decoding.")
    ...
    parser.add_argument("--spec-algo", type=str, default="STANDALONE", choices=["STANDALONE", "EAGLE"], 
    ...
    parser.add_argument("--output_len", type=int, default=256, help="New tokens to generate per request")
    ...
    parser.add_argument("--gpu-mem-util", type=float, default=0.65, help="GPU memory utilization (0-1)")
    ...
    def _set_cuda_visible_devices_from_args(args) -> None:
        ...
        if args.devices:
            devs = ",".join([d.strip() for d in args.devices.split(",") if d.strip() != ""])
            if devs:
        ...
        if args.gpus > 1 and (args.auto_select_gpus or not args.devices):
            try:
                ...
                if len(selected) >= args.gpus:
                    ...
                    print(f"Using GPUs (auto-selected by free memory): {os.environ['CUDA_VISIBLE_DEVICES']}")
                pynvml.nvmlShutdown()
            except Exception as e:
                print(f"Warning: GPU auto-selection failed: {e}. Proceeding with default device visibility.")
    ...
    try:
        ...
        print(f"Error initializing SGLang engine: {e}")
        sys.exit(1)
    ...
    if string_prompts is not None:
        ...
        prompts = [tokenizer.decode(token_ids, skip_special_tokens=True) for token_ids in prompt_token_ids]
    else:
        raise ValueError("No prompts generated")
    
    print(f"Generated {len(prompts)} prompts")
    if args.verbose and len(prompts) > 0:
    ...
    try:
        ...
        for i, (prompt, output) in enumerate(zip(prompts, outputs)):
            ...
            completion_tokens = len(tokenizer.encode(completion_text)) - prompt_tokens
        ...
        if args.spec:
            print(f"Speculative decoding enabled (algo={args.spec_algo}, k={args.k})")
