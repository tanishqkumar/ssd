...
# Cache for vectorized path precomputed components
_vec_cache = {}

@torch.inference_mode()
def get_custom_mask_vectorized(config, context_lens, step: int, K: int, B: int, device: torch.device, cache_hits: torch.Tensor):
    """Vectorized mask generation for B>8. Supports non-uniform fan_out_list.

    Uses flat_blocks_after_cat with hit glue for all batch elements,
    then fixes up glue columns for cache-miss elements when hit/miss differ.
    """
    global _vec_cache

    cache_key = (K, tuple(config.fan_out_list), tuple(config.fan_out_list_miss), device)
    if _vec_cache.get('key') != cache_key:
        fan_out_t = torch.tensor(config.fan_out_list, device=device, dtype=torch.long)
        fan_out_t_miss = torch.tensor(config.fan_out_list_miss, device=device, dtype=torch.long)
        MQ_LEN = int(fan_out_t.sum().item())

        tril = torch.tril(torch.ones(K+1, K+1, device=device)).to(torch.bool)
        glue_hit = tril[torch.arange(K+1, device=device).repeat_interleave(fan_out_t)]
        glue_miss = tril[torch.arange(K+1, device=device).repeat_interleave(fan_out_t_miss)]
    ...
    assert torch.all(prefix_lens >= 0), f"prefix_lens must be non-negative, got {prefix_lens}"

    # Main vectorized path: build mask with hit glue for all batch elements
    out = flat_blocks_after_cat(prefix_lens, M_hit)

    # Fixup: overwrite glue columns for cache-miss batch elements
    if c['glue_differs']:
        miss_mask = (cache_hits == 0)
        if miss_mask.any():
            y = M_hit.shape[1]
            cols_per_block = prefix_lens.to(torch.long) + y
            elems_per_block = MQ_LEN * cols_per_block
            block_starts = torch.zeros(B + 1, device=device, dtype=torch.long)
            block_starts[1:] = elems_per_block.cumsum(0)
