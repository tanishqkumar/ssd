...
def load_embedding_from_target(
    model: nn.Module,
    target_path: str,
    ...
    if target_hidden_size is not None and draft_hidden_size is not None:
        if target_hidden_size != draft_hidden_size:
            print(
                f"[load_model] Skipping target embeddings: target hidden_size={target_hidden_size} != draft hidden_size={draft_hidden_size}"
            )
            return False
...
def load_eagle_model(
    ...
    if safetensor_files:
        ...
        # Load all safetensors into a single state dict (handles sharded checkpoints)
        state_dict = {}
        for file in safetensor_files:
            try:
                with safe_open(file, "pt", "cpu") as f:
                ...
                print(
                    f"[load_model] Loaded {len(state_dict)} cumulative weights (latest file: {file})"
                )
            except Exception as e:
                print(f"[load_model] Error reading safetensor {file}: {e}")
                continue
    ...
    if state_dict is None or len(state_dict) == 0:
        ...
        if not os.path.exists(bin_file):
            raise FileNotFoundError(
                f"No safetensors or pytorch_model.bin found at {path} for EAGLE3 draft model"
            )
    ...
    if hasattr(model, "d2t") and "d2t" in state_dict:
        ...
        d2t_values = d2t_tensor.to(dtype=torch.int64, device="cpu").tolist()
        model.d2t = {i: value for i, value in enumerate(d2t_values)}
        model.d2t_tensor = d2t_tensor.to(
            "cuda"
        ).long()  # keep as tensor for fast indexing
    ...
    if hasattr(model, "t2d") and "t2d" in state_dict:
        ...
        t2d_values = t2d_tensor.to(dtype=torch.int64, device="cpu").tolist()
        model.t2d = {i: value for i, value in enumerate(t2d_values)}
        model.t2d_tensor = t2d_tensor.to(
            "cuda"
        ).long()  # keep as tensor for fast indexing
    ...
    if not found_embed_tokens:
        if target_path and target_hidden_size is not None:
            ...
            print(
                f"[load_model] 'embed_tokens' not found in draft weights. Attempting to load from target path: {target_path}"
            )
            if load_embedding_from_target(
                model, target_path, target_hidden_size, draft_hidden_size
            ):
                found_embed_tokens = True
        elif target_path:
    ...
    # Load model weights
    for weight_name, loaded_weight in tqdm(
        state_dict.items(), desc="Loading EAGLE3 weights"
    ):
        ...
        print(
            f"[load_model] loading weight {weight_name} with shape {loaded_weight.shape}"
        )
...
def load_model(
    model: nn.Module, path: str, target_path: str = None, target_hidden_size: int = None
):
