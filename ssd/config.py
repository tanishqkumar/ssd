...
def _default_target() -> str:
    from ssd.paths import DEFAULT_TARGET

    return DEFAULT_TARGET


def _default_draft() -> str:
    from ssd.paths import DEFAULT_DRAFT
        ...
        if self.speculate:
            ...
            if self.draft_async:
                ...
                assert sum(self.fan_out_list_miss) == sum(self.fan_out_list), (
                    "ERROR in Config: fan_out_list_miss must be the same as fan_out_list"
                )
        ...
        if self.use_eagle:
            if self.eagle_layers is None:
                ...
                self.eagle_layers = [
                    ...
                    L // 2,
                    L - 3,
                ]  # [2, 16, 29] outputs, ie. [3, L//2+1, L-2] inputs
            ...
            if self.speculate and self.draft_hf_config is not None:
                ...
                if target_rope_theta != draft_rope_theta:
                    print(
                        f"[Config] Overriding eagle draft rope_theta: {draft_rope_theta} -> {target_rope_theta}",
                        flush=True,
                    )
                ...
                target_max_pos = getattr(
                    self.hf_config, "max_position_embeddings", 8192
                )
                draft_max_pos = getattr(
                    self.draft_hf_config, "max_position_embeddings", 2048
                )
                if target_max_pos != draft_max_pos:
                    print(
                        f"[Config] Overriding eagle draft max_position_embeddings: {draft_max_pos} -> {target_max_pos}",
                        flush=True,
                    )
